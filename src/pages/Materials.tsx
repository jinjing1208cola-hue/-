...
export default function Materials() {
  ...
  const handleBatchCategory = async (newCat: string) => {
    for (const id of selected) {
      ...
      if (m) { m.category = newCat; await updateMaterial(m) }
    }
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 className="page-title" style={{ marginBottom: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
            <FolderOpen size={26} strokeWidth={1.8} style={{ color: 'var(--primary)' }} />
            物料库
          </h1>
          <p className="page-subtitle" style={{ marginTop: 4, marginBottom: 0 }}>
            管理产品图片、教程视频、使用手册等运营物料
          </p>
        </div>
      ...
      {selected.size > 0 && (
        <div className="batch-bar">
          ...
          <button className="btn" onClick={() => setBatchCatOpen(true)}>
            <FolderOpen size={14} /> 批量改分类
          </button>
          <button className="btn" onClick={handleBatchDelete} style={{ color: 'var(--danger)' }}>
            <Trash2 size={14} /> 批量删除
          </button>
      ...
      <Modal open={batchCatOpen} onClose={() => setBatchCatOpen(false)} title="批量修改分类" width={360}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {CATEGORIES.map(c => (
            <button key={c} className="btn" style={{ justifyContent: 'flex-start' }}
              onClick={() => handleBatchCategory(c)}>
      ...
      <div className="toolbar" style={{ marginBottom: 20, gap: 8 }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
          ...
          <input className="search-input" placeholder="搜索名称或备注..." value={search}
        ...
        <div className="view-toggle">
          <button className={`btn ${viewMode === 'grid' ? 'btn-primary' : ''}`}
            onClick={() => setViewMode('grid')}><LayoutGrid size={15} /></button>
          <button className={`btn ${viewMode === 'list' ? 'btn-primary' : ''}`}
        ...
        {!uploading ? (
          <>
            ...
            <div className="form-group">
              ...
              <textarea className="form-textarea" value={uploadNotes}
                onChange={e => setUploadNotes(e.target.value)}
                placeholder="可选，记录该物料的用途或说明..." rows={2} />
            </div>
            <div
      ...
      <Modal open={!!previewItem} onClose={() => setPreviewItem(null)}
        title={previewItem?.name || ''} width={860}>
        {previewItem && (
          <div className="preview-content">
            ...
            {previewItem.type === 'video' && (
              <video src={previewItem.dataUrl} controls className="preview-media" />
            )}
            {previewItem.type === 'pdf' && (
              <iframe src={previewItem.dataUrl} className="preview-pdf-iframe"
                title={previewItem.name} />
            )}
            <div className="preview-meta">
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><CalendarIcon size={14} /> {formatDate(previewItem.uploadedAt)}</span>
            ...
            {previewItem.notes && (
              <div className="preview-notes" style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
        ...
        <div className="material-grid">
          {filtered.map(m => (
            <div key={m.id} className={`material-card ${selected.has(m.id) ? 'selected' : ''}`}>
              <div className="material-thumb" onClick={() => setPreviewItem(m)}>
                {m.type === 'image' ? (
                  <img src={m.dataUrl} alt={m.name} loading="lazy" />
                ) : m.type === 'video' ? (
                  <div className="material-thumb-video">
                    <video src={m.dataUrl} muted preload="metadata" />
                    <div className="play-overlay">▶</div>
                  </div>
                ) : (
                  <div className="material-thumb-pdf">
                    <FileText size={48} strokeWidth={1.2} style={{ color: '#f43f5e' }} />
                  </div>
                )}
                <label className="material-check" onClick={e => e.stopPropagation()}>
              ...
              <div className="material-info">
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
                  <select className="material-category-select" value={m.category}
                  ...
                  <button className="btn" style={{ fontSize: 11, padding: '4px 8px' }}
                    onClick={(e) => {
                      e.stopPropagation()
                      const notes = prompt('编辑备注', m.notes) 
                      if (notes !== null) handleUpdateNotes(m.id, notes)
                    }}>
                    <MessageSquare size={12} />
                  </button>
                </div>
              </div>
              <button className="material-delete" onClick={() => {
                deleteMaterials([m.id])
        ...
        <div className="card">
          <div className="card-body" style={{ padding: 0, overflowX: 'auto' }}>
            <table className="data-table">
              ...
              <tbody>
                {filtered.map(m => (
                  <tr key={m.id} style={{ cursor: 'pointer' }} onClick={() => setPreviewItem(m)}>
                    ...
                    <td style={{ maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13, color: 'var(--text-secondary)' }}>
                      {m.notes || '—'}
                    </td>
                    <td style={{ color: 'var(--text-muted)', fontSize: 13 }}>{formatDate(m.uploadedAt)}</td>
                    <td onClick={e => e.stopPropagation()}>
                      <button className="material-delete" style={{ position: 'static', opacity: 1 }}
                        onClick={() => {
                          deleteMaterials([m.id])
                          setMaterials(prev => prev.filter(x => x.id !== m.id))
